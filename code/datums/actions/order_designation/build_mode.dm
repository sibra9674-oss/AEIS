...
/obj/effect/build_designator/Initialize(mapload, obj/construct_type, mob/builder) //construct_type is a TYPE but typecast for initial values below
	...
	recipe = get_designator_recipe(construct_type)
	if(!recipe)
		return INITIALIZE_HINT_QDEL
...
/obj/effect/build_designator/proc/get_designator_recipe(obj/construct_type)
	switch(construct_type)
		if(/obj/structure/barricade/solid)
			return new /datum/stack_recipe("metal barricade", /obj/structure/barricade/solid, 4, time = 6 SECONDS, crafting_flags = CRAFT_CHECK_DENSITY | CRAFT_CHECK_DIRECTION | CRAFT_ON_SOLID_GROUND, skill_req = SKILL_CONSTRUCTION_METAL)
		if(/obj/structure/barricade/solid/plasteel)
			return new /datum/stack_recipe("plasteel barricade", /obj/structure/barricade/solid/plasteel, 4, time = 6 SECONDS, crafting_flags = CRAFT_CHECK_DENSITY | CRAFT_CHECK_DIRECTION | CRAFT_ON_SOLID_GROUND, skill_req = SKILL_CONSTRUCTION_PLASTEEL)
		if(/obj/structure/barricade/folding/metal)
			return new /datum/stack_recipe("metal folding barricade", /obj/structure/barricade/folding/metal, 6, time = 10 SECONDS, crafting_flags = CRAFT_CHECK_DENSITY | CRAFT_CHECK_DIRECTION | CRAFT_ON_SOLID_GROUND, skill_req = SKILL_CONSTRUCTION_METAL)
		if(/obj/structure/barricade/folding)
			return new /datum/stack_recipe("plasteel folding barricade", /obj/structure/barricade/folding, 6, time = 10 SECONDS, crafting_flags = CRAFT_CHECK_DENSITY | CRAFT_CHECK_DIRECTION | CRAFT_ON_SOLID_GROUND, skill_req = SKILL_CONSTRUCTION_PLASTEEL)
